...
For non-model-based methods only oneformalization method~\acrlong{acn:ltl}, described in~\ref{sec:sub:ltl}, is investigated.
...
But the authors don't provide a formal verification of the transformation and argue with the \enquote{obvious correctness} of the transformation.
...
This~\acrshort{acn:dsl} is then verified by transforming it into an~\acrshort{acn:ltl} formula and using a model checker.
...
This approach enables a thorough verification and ease validation.
...
Through a study~\cite{6315074} they concluded that the plcML approach was too complex and required a lot of specialized training in order to be more effective than traditional coding with the IEC 61131-3 languages.
